...
#if (I3L_LIGHTMODEL == I3L_LIGHTMODEL_CUSTOM)
				...
				float litted)
{
	...
	float3 reverseColor = saturate( dot( -lightDir, Normal)) * float3( 0.1f, 0.1f, 0.1f) * diffColor;
	
	return (((litted  * diffColor * secondColor) + sColor) * litDiffuse) + (reverseColor) * float3( 0.0f, 0.0f, 1.0f);
}
#endif