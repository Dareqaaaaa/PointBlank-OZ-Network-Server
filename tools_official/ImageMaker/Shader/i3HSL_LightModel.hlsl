...
#if (I3L_LIGHTMODEL == I3L_LIGHTMODEL_HSL)
				...
				float litted)
{
	...
	float3 reverseColor = saturate( dot( -lightDir, Normal)) * float3( 0.1f, 0.5f, 0.1f) * diffColor;
	
	return (((litted  * diffColor * secondColor) + sColor) * litDiffuse) + (reverseColor);
}
#endif